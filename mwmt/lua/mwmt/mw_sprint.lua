...
if CLIENT then
    ...
    local t, tx, ty, tz = Tween(0, 60, 0.5, ease), Tween(0, tvec.x, 0.5, ease), Tween(0, tvec.y, 0.5, ease), Tween(0, tvec.z, 0.5, ease)

    local twohanded_table = {"mg_sbeta","mg_acharlied","mg_ngsierra","mg_tsierra12","mg_romeo700",
    "Sniper Rifles", "mg_kilo98", "mg_mike26", "mg_romeo870", "mg_dblmg"}
    local onehanded_table = {"mg_delta"}

    local vec_table = {["mg_lima86"] = Vector(-3,-5,-10), ["mg_akilo47"] = Vector(0,-5,-10), ["mg_akilo545"] = Vector(0,-3,-10), ["mg_akilo762"] = Vector(0,-3,-10),
    ["mg_mike4"] = Vector(-1.5,-3,-10), ["mg_mike4miiw"] = Vector(-1.5,-3,-15), ["mg_ngsierra"] = Vector(0,-10,-22), ["mg_tango21"] = Vector(-3,-3,-11), ["mg_tango21_mw3"] = Vector(-3,-3,-11),
    ...
    hook.Add( "CalcViewModelView", "sprint_calcvm", function(wep,vm,opos,oang,pos,ang)
        ...
        ang:RotateAroundAxis(ang:Right(),t:GetValue()) ang:RotateAroundAxis(ang:Forward(),3)
        return pos + off, ang
    end)
